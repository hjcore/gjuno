# To build the GJuno image, just run:
# > docker build -t gjuno .
#
...
#
# Simple usage with a mounted data directory (considering ~/.gjuno/config as the configuration folder):
# > docker run -it -v ~/.gjuno/config:/home gjuno gjuno parse config.toml firebase-config.json
#
# If you want to run this container as a daemon, you can do so by executing
# > docker run -td -v ~/.gjuno/config:/home --name gjuno gjuno
#
# Once you have done so, you can enter the container shell by executing
# > docker exec -it gjuno bash
#
...
RUN cp /code/build/gjuno /usr/bin/gjuno
RUN echo "Ensuring binary is statically linked ..." && (file /usr/bin/gjuno | grep "statically linked")
