...
start-docker-test: stop-docker-test
	...
	@docker run --name gjuno-test-db -e POSTGRES_USER=gjuno -e POSTGRES_PASSWORD=password -e POSTGRES_DB=gjuno -d -p 6432:5432 postgres
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' -not -path "./venv" | xargs goimports -w -local github.com/hjcore/gjuno
.PHONY: format
